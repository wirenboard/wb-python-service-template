...
Локально перед `pytest` нужен один шаг, `python3 setup.py --quiet egg_info`. Тесты версии читают метаданные пакета, а вне сборки их никто не создает, поэтому без этого шага два теста падают с `PackageNotFoundError`. На сборке то же самое делает хук `PYBUILD_BEFORE_TEST` из `debian/rules`.
