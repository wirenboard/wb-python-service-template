...
- **Для создания mqtt клиента** используйте `MQTTClient` (на базе paho) из библиотеки [wb_common](https://github.com/wirenboard/wb-common/blob/master/wb_common/mqtt_client.py). Вы можете использовать функцию клиента start(), которая по умолчанию запускает клиента в [отдельном потоке](wb/python_service_template/main.py#L97). Также вы можете выключить создание отдельного потока и вручную запускать клиента через start() + loop_forever() в [текущем потоке](wb/python_service_template/main.py#L37). Использовать функцию loop() для работы не рекомендуется, тк она не делает автоматический реконнект в случае разрыва соединения с брокером.
...
2. Код в рантайме зовет `get_version()`, который читает версию из метаданных установленного пакета через `importlib.metadata`. Искать файлы рядом с собой не нужно, поэтому одна и та же строка доступна и CLI, и сервису, и любому библиотечному коду.
...
Метаданные пакета появляются только на стадии install, уже после тестов. Поэтому в `debian/rules` стоят два хука pybuild, которые создают их в сборочном каталоге перед тестами и убирают после, чтобы стадия тестов оставляла дерево в том же виде, в каком его нашла.
...
**Доктесты включены по умолчанию** через `tests/test_doctests.py`, который прогоняет доктесты всех модулей пакета. Пример — `parse_changelog_version()` в `version.py`, где примеры вызова документируют разбор строки changelog внятнее, чем это сделал бы комментарий. Доктесты уместны ровно там, где пример и есть самая понятная документация функции, и падают вместе со сборкой, если поведение разошлось с примером.

Прогоняем их обычным тестом, а не штатным `--doctest-modules`, потому что pybuild зовет `pytest tests` с явным путем, а явный путь перебивает и `testpaths`, и пути из `addopts`. Обычный тест собирается при любом способе запуска, поэтому отдельный конфиг pytest шаблону не нужен.

Проверки, у которых нет потребителя в рабочем коде, живут в тестах, а не в пакете. Формат версии проверяет `tests/test_version.py`.
...
В самом шаблоне такого файла нет намеренно — у шаблонного CLI всего два флага, и автодополнение на них не окупается. Генерировать через `shtab` тоже стоит только при реальной необходимости, иначе это лишняя build-зависимость и шаг генерации ради пары строк.
...
- **Jenkinsfile**
  ...
  В шаблоне вызов пайплайна закомментирован намеренно — иначе шаблон собирался бы сам и уезжал в релизный фид, где ему делать нечего. Первым делом при работе над настоящим пакетом строки надо раскомментировать.
  ...
  `defaultRunPythonChecks` включает прогон codestyle, `defaultRunLintian` — проверку собранного deb, `defaultAngryPylint` заставляет сборку падать на ошибках pylint, а не только показывать их в отчете.
...
- **Для чего при сборке cdeb запускаются и `--arch-all`, и `--arch-any`?**
  ...
  _Так получилось_, что при переходе на релизную систему, когда для сборки
