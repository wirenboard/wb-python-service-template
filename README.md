...
### Сервис
 ...
 - **Для создания mqtt клиента** используйте `MQTTClient` (на базе paho) из библиотеки [wb_common](https://github.com/wirenboard/wb-common/blob/master/wb_common/mqtt_client.py). Оба варианта запуска показаны в [main.py](wb/python_service_template/main.py). Функция клиента start() по умолчанию запускает его в отдельном потоке, так сделан класс `ThreadedServiceTemplate`. Создание отдельного потока можно выключить и запускать клиента вручную через start() + loop_forever() в текущем потоке, так сделан класс `OneThreadServiceTemplate`. Использовать функцию loop() для работы не рекомендуется, тк она не делает автоматический реконнект в случае разрыва соединения с брокером. 
 ...
 - **Виртуальные устройства снимает сам сервис.** Свои устройства сервис публикует с `retain`, поэтому после остановки они остаются на брокере и продолжают висеть в интерфейсе. Убрать их можно единственным способом, отправив в те же топики пустое значение, и сделать это может только сам сервис.

   Снятие ставится на штатный путь остановки. Обработчик сигнала выставляет флаг, основной цикл завершается, сервис затирает свои топики и только после этого отключается от брокера. В `finally` этот код писать нельзя, иначе он сработает и при падении с необработанным исключением.

   При аварийной смерти устройства должны остаться на месте и показывать ошибку, чтобы поломка была видна. Для этого сервис до подключения регистрирует Last Will с записью ошибки в `meta/error`, у самого устройства или у любого его контрола. Брокер отправит её сам, когда потеряет клиента. При штатном отключении Last Will не срабатывает, так брокер и отличает падение от нормальной остановки.
...
**Аннотации типов обязательны** — у всех параметров и у всех возвращаемых значений, включая `-> None`. Требование записано в [codestyle](https://github.com/wirenboard/codestyle).
...
Читатель на странице репозитория первым делом хочет понять, зачем сервис нужен. Поэтому README начинается с одного-двух предложений о том, что это за сервис, а сразу за ними идёт раздел `## Зачем это нужно`. Сначала абзац о том, как та же задача решается без сервиса и что в ней человек делает руками, потом нумерованный список, где каждый пункт начинается с жирной фразы про снятую проблему, а не пересказывает функцию.

Образцы — [wb-docker-ce-repack](https://github.com/wirenboard/wb-docker-ce-repack), [wb-node-red](https://github.com/wirenboard/wb-node-red), [wb-mqtt-waterius](https://github.com/wirenboard/wb-mqtt-waterius).
